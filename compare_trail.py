#!/usr/bin/env python3
"""Compare fixed SL vs trailing SL at 2% on 1H EMA(100/200) 3-year backtest."""
import sys
...
print(f"  {len(hourly):,} 1H bars\n")

def run(label, sl_pct, be_rr, trailing):
    ...
    print(f"  {label} done")
    return dict(trades=len(closed), winners=len(w), losers=len(l),
...
configs = [
    ("Fixed SL 0.5%",        0.005, 0.0),
    ("Fixed SL 2.0%",        0.020, 0.0),
    ("Trailing SL 2.0%",     0.020, 0.0),   # trailing is always on now
]

print("Running comparisons...")
r = {}
r["Fixed SL 0.5%"]    = run("Fixed SL 0.5%",    0.005, 0.0, False)
r["Fixed SL 2.0%"]    = run("Fixed SL 2.0%",    0.020, 0.0, False)
r["Trailing SL 2.0%"] = run("Trailing SL 2.0%", 0.020, 0.0, True)

keys = list(r.keys())
print()
print("=" * 72)
print(f"  {'':22}  {'Fixed 0.5%':>14}  {'Fixed 2.0%':>14}  {'Trail 2.0%':>14}")
print("=" * 72)
for metric, fmt, label in [
    ...
    ("losers",   "{:>14}",    "Losers"),
    ("wr",       "{:>13.1f}%","Win Rate"),
    ("avg_win",  "{:>+14.2f}","Avg Win (USDT)"),
    ("avg_loss", "{:>+14.2f}","Avg Loss (USDT)"),
    ("max_win",  "{:>+14.2f}","Best Trade (USDT)"),
    ("max_loss", "{:>+14.2f}","Worst Trade (USDT)"),
    ("pf",       "{:>14.2f}", "Profit Factor"),
    ("net",      "{:>+14.2f}","Net PnL (USDT)"),
    ("pct",      "{:>+13.2f}%","Return"),
]:
    row = f"  {label:<22}"
    for k in keys:
        row += "  " + fmt.format(r[k][metric])
    print(row)
print("=" * 72)
